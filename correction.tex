...
which also depends on the number of degrees of freedom, $N_{\mathrm{dof}}$.
A new $\chi^{\prime 2}$
value can now be obtained, namely that which would give the same p-value but
with a different number of degrees of freedom $N_{\mathrm{dof}}^\prime$,
...
\begin{displaymath}
p = F(\chi^2,N_{\mathrm{dof}})\qquad\mathrm{so}\qquad
\chi^2 = F^{-1}(p,N_{\mathrm{dof}}),
\end{displaymath}
Hence, the new $\chi^{\prime 2}$ value which gives the same p-value with the
new $N_{\mathrm{dof}}^\prime$ is given by
\begin{displaymath}
\chi^{\prime 2} = F^{-1}(p,N_{\mathrm{dof}}^\prime)
\end{displaymath}
and the corrected \nll is then given by
\begin{displaymath}
...
this correction was applied even in cases with lower statistics,
